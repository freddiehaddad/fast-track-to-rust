...
`Crates.toml` and include it in `main.rs` with `use itertools::Itertools`. Let's
see how the [`coalesce`] adaptor can simplify the code:
...
fn create_intervals(
    ...
    match_lines
        ...
        .map(|line| {
            ...
            let end = (lines.len() - 1).min(line.saturating_add(after_context));
